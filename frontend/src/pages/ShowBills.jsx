            ...
            {bills.map((bill,index) => {
              return (
                <tr class="border-b">
                  ...
                  <td class="px-4 py-2 text-gray-700">{bill.date.substring(0, 10)}</td>
                  <td class="px-4 py-2 text-gray-700">{bill.customer.name}</td>
                  <td class="px-4 py-2 text-gray-700">{bill.location.address}</td>
                  <td class="px-4 py-2 text-gray-700">{bill.tax.netAmount}</td>
                  <td class="px-4 py-2 text-gray-700">
                    <button 
                    onClick={()=>{
                      localStorage.setItem("billForEdit",JSON.stringify(bill));
                      navigate("/")
                    }}
                    class="text-blue-500 hover:text-blue-700">
                      Edit
                    </button>
                    <button onClick={()=>handleDelete(bill.number,index)} class="ml-2 text-red-500 hover:text-red-700">
                      Delete
                    </button>
                  </td>
                </tr>
